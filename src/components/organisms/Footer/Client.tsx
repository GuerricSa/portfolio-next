'use client'

import { usePathname } from 'next/navigation';
import Link from 'next/link';
import { MouseEvent } from 'react';
import { useContactModal } from 'context/ContactModalContext';
import ScrollToTopButton from 'components/molecules/ScrollToTopButton';

export default function FooterClient() {
  const pathname = usePathname();
  const { openModal } = useContactModal();

  return (
    <footer className="footer text-primary pt-8 mt-12" role="contentinfo">
      <div className="container">
        <div className='flex flex-wrap items-center justify-between gap-4 relative'>
          <div className="text-3xl font-bold lg:w-80">
            <Link
              href="/"
              onClick={(e: MouseEvent<HTMLAnchorElement>) => {
                if (pathname === "/") {
                  e.preventDefault();
                  window.scrollTo({ top: 0, behavior: 'smooth' });
                }
              }}
              className="text-primary flex logo--hover-effect w-fit"
              onMouseEnter={(e: MouseEvent<HTMLAnchorElement>) => {
                const link = e.currentTarget;
                if (!link.classList.contains('animate')) {
                  link.classList.add('animate');
                  setTimeout(() => {
                    link.classList.remove('animate');
                  }, 1400);
                }
              }}
              aria-label="Retour à l'accueil"
            >
              <span className="inline-block">G</span>
              <span className="inline-block">S</span>
              <span className="text-tertiary inline-block">.</span>
            </Link>
          </div>
          <ScrollToTopButton />
          <nav className="py-4 justify-end flex" aria-label="Liens de contact">
            <button
              onClick={openModal}
              className="block py-2 group px-4"
              aria-label="Me contacter par email"
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path className="group-hover:fill-tertiary transition-[fill] duration-300 ease-in-out" d="M23.938 5.26291C23.8298 4.71795 23.5882 4.21924 23.2507 3.80726C23.1796 3.71749 23.1052 3.6371 23.0247 3.55671C22.4268 2.95587 21.5876 2.58105 20.6709 2.58105H3.32911C2.41238 2.58105 1.57622 2.95587 0.975516 3.55671C0.895125 3.6371 0.820781 3.71749 0.749437 3.80726C0.411797 4.2192 0.170156 4.71795 0.0649688 5.26291C0.02175 5.47324 0 5.6906 0 5.91035V18.09C0 18.5573 0.0990937 19.0064 0.275812 19.412C0.439734 19.7992 0.681422 20.1496 0.975469 20.4433C1.05 20.5177 1.12402 20.586 1.20478 20.654C1.78069 21.1309 2.52408 21.419 3.32911 21.419H20.6709C21.4762 21.419 22.2228 21.1309 22.7954 20.651C22.8759 20.586 22.9503 20.5177 23.0247 20.4433C23.3186 20.1496 23.5602 19.7992 23.7275 19.412V19.409C23.9042 19.0034 24 18.5573 24 18.0901V5.91035C24 5.6906 23.9786 5.47324 23.938 5.26291ZM2.18011 4.76116C2.47753 4.46407 2.87695 4.28421 3.32911 4.28421H20.6709C21.123 4.28421 21.526 4.46407 21.8199 4.76116C21.8725 4.81409 21.922 4.87305 21.9658 4.93165L12.8764 12.8533C12.6257 13.073 12.316 13.1815 12 13.1815C11.6872 13.1815 11.3777 13.073 11.1237 12.8533L2.03775 4.92827C2.07797 4.86968 2.12752 4.81409 2.18011 4.76116ZM1.70316 18.09V6.69073L8.28084 12.4289L1.70648 18.161C1.70316 18.1393 1.70316 18.1148 1.70316 18.09ZM20.6709 19.7155H3.32911C3.03469 19.7155 2.7592 19.6381 2.52408 19.5021L9.46055 13.4572L10.1081 14.0205C10.65 14.4914 11.3282 14.7299 12 14.7299C12.6753 14.7299 13.3534 14.4914 13.8953 14.0205L14.5425 13.4572L21.4763 19.5021C21.2408 19.6381 20.9653 19.7155 20.6709 19.7155ZM22.2968 18.09C22.2968 18.1148 22.2968 18.1393 22.2937 18.161L15.7193 12.4323L22.2968 6.69377V18.09Z" fill="#222222"/>
              </svg>
            </button>
            <a
              target="_blank"
              rel="noopener noreferrer"
              href="https://www.linkedin.com/in/guerric-sant/"
              className="block py-2 group px-4"
              aria-label="Voir mon profil LinkedIn (s'ouvre dans un nouvel onglet)"
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path className="group-hover:stroke-tertiary transition-[stroke] duration-300 ease-in-out" d="M18.25 3.75H5.75C4.64543 3.75 3.75 4.64543 3.75 5.75V18.25C3.75 19.3546 4.64543 20.25 5.75 20.25H18.25C19.3546 20.25 20.25 19.3546 20.25 18.25V5.75C20.25 4.64543 19.3546 3.75 18.25 3.75Z" stroke="#222222" strokeWidth="1.5"/>
                <path className="group-hover:stroke-tertiary transition-[stroke] duration-300 ease-in-out" d="M8.25 10.75V16.25" stroke="#222222" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                <path className="group-hover:stroke-tertiary group-hover:fill-tertiary transition-[stroke, fill] duration-300 ease-in-out" d="M9.1875 8C9.1875 8.51777 8.76777 8.9375 8.25 8.9375C7.73223 8.9375 7.3125 8.51777 7.3125 8C7.3125 7.48223 7.73223 7.0625 8.25 7.0625C8.76777 7.0625 9.1875 7.48223 9.1875 8Z" fill="#222222" stroke="#222222" strokeWidth="0.125"/>
                <path className="group-hover:stroke-tertiary transition-[stroke] duration-300 ease-in-out" d="M15.75 16.25V13C15.75 11.7574 14.7426 10.75 13.5 10.75C12.2574 10.75 11.25 11.7574 11.25 13V16.25" stroke="#222222" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </a>
            <a
              target="_blank"
              rel="noopener noreferrer"
              href="https://github.com/GuerricSa"
              className="block py-2 group px-4"
              aria-label="Voir mon profil GitHub (s'ouvre dans un nouvel onglet)"
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path className="group-hover:fill-tertiary transition-[fill] duration-300 ease-in-out" d="M18.27 6.96003C18.63 5.67003 18.09 4.26003 18.06 4.17003C17.97 3.96003 17.76 3.78003 17.52 3.78003C17.46 3.78003 16.08 3.66003 14.7 4.71003C13.62 4.50003 12.39 4.44003 12.33 4.44003C12.3 4.44003 12.3 4.44003 12.27 4.44003C12.21 4.44003 10.98 4.50003 9.90004 4.71003C8.52004 3.66003 7.14004 3.78003 7.08004 3.78003C6.84004 3.81003 6.63004 3.96003 6.54004 4.17003C6.51004 4.23003 5.94004 5.67003 6.33004 6.96003C5.64004 7.92003 5.10004 9.18003 5.58004 11.43C6.03004 13.47 7.71004 14.61 9.45004 15C9.30004 15.42 9.24004 15.93 9.18004 16.47C8.46004 16.5 8.01004 16.08 7.38004 15.42C6.84004 14.85 6.24004 14.19 5.25004 13.98C4.92004 13.89 4.59004 14.1 4.50004 14.46C4.44004 14.82 4.65004 15.15 5.01004 15.21C5.58004 15.33 5.97004 15.75 6.45004 16.26C7.08004 16.92 7.80004 17.73 9.09004 17.73H9.12004C9.12004 18.42 9.18004 19.08 9.21004 19.62C9.24004 19.98 9.54004 20.22 9.90004 20.19C10.26 20.16 10.5 19.86 10.47 19.5C10.29 17.46 10.44 15.3 10.77 15C11.01 14.85 11.16 14.58 11.07 14.28C11.01 14.01 10.77 13.8 10.47 13.8C10.2 13.8 7.38004 13.65 6.84004 11.16C6.39004 9.12003 6.96004 8.22003 7.56004 7.50003C7.71004 7.32003 7.74004 7.05003 7.65004 6.81003C7.41004 6.30003 7.50004 5.58003 7.62004 5.10003C8.04004 5.16003 8.67004 5.34003 9.33004 5.91003C9.48004 6.06003 9.69004 6.09003 9.90004 6.06003C10.83 5.82003 12.09 5.76003 12.3 5.76003C12.48 5.76003 13.77 5.82003 14.7 6.06003C14.91 6.12003 15.12 6.06003 15.27 5.91003C15.93 5.34003 16.59 5.16003 16.98 5.10003C17.1 5.55003 17.19 6.27003 16.95 6.81003C16.86 7.02003 16.89 7.29003 17.04 7.50003C17.64 8.22003 18.18 9.12003 17.76 11.16C17.22 13.65 14.4 13.8 14.07 13.83C13.77 13.83 13.53 14.04 13.47 14.31C13.41 14.58 13.53 14.88 13.77 15.03C14.13 15.33 14.28 17.46 14.1 19.53C14.07 19.89 14.34 20.19 14.67 20.22C14.7 20.22 14.7 20.22 14.73 20.22C15.06 20.22 15.33 19.98 15.36 19.65C15.48 18.33 15.57 16.29 15.15 15C16.89 14.64 18.57 13.47 19.02 11.43C19.5 9.18003 18.99 7.92003 18.27 6.96003Z" fill="#222222"/>
              </svg>
            </a>
          </nav>
        </div>
        <div className="text-center border-t-2 border-zinc-500 md:text-left py-4 flex justify-between flew-wrap px-4 lg:px-8">
          <p className="text-xs"> &copy; {new Date().getFullYear()} Guerric SANT. All rights reserved</p>
          <Link
            href="/cgv"
            className="text-xs underline"
            aria-label="Lire les conditions générales de vente"
          >
            CGV
          </Link>
        </div>
      </div>
    </footer>
  );
}
